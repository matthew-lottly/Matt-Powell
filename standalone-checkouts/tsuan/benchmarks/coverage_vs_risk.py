...
def run_benchmark(
    ...
    for idx in range(len(ds)):
        sample = ds[idx]
        x_opt = sample["optical"].unsqueeze(0).to(device)
        x_sar = sample["sar"].unsqueeze(0).to(device)
        u_mask = sample["cloud_mask"].unsqueeze(0).to(device)

        with torch.no_grad():
            out = model(x_opt, x_sar, u_mask)

        x_hat = out["x_hat"].cpu().numpy()[0]  # (T, C, H, W)
        x_target = sample["optical"].numpy()  # (T, C, H, W)
