...
from sports_sim.core.models import Player, Team, GameState, SimulationConfig
...
def make_team(name: str) -> Team:
    ...
    t = Team(name=name, abbreviation=name[:3].upper(), players=[p1], bench=[p2])
    return t
...
def test_realism_smoke_basic():
    ...
    state = GameState(sport="soccer", home_team=home, away_team=away)
    ...
    # momentum update with a fake goal event
    from sports_sim.core.models import GameEvent, EventType
