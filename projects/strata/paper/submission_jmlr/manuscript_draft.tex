...
For practitioners we provide concise recommendations:
\begin{itemize}
...
\item \textbf{CHMP (propagation-aware):} prefer when you need spatial
redistribution of interval budget (e.g., to allocate more conservatism near
known high-error neighborhoods) while preserving conformal coverage.
\item \textbf{Ensemble Calibrator:} use when interval sharpness is critical and
you can afford $3\times$ training cost; this was the only variant that produced a
statistically significant aggregate width reduction in our experiments.
\end{itemize}

\paragraph{Choosing $\alpha$ and monitoring.} Pick $\alpha$ to set the
target nominal coverage (for example, $\alpha=0.10$ for 90\% coverage), then
inspect spatial width maps to ensure critical locations receive adequate
interval budget. In multi-type deployments monitor per-type coverage and the
worst-group gap; triggers for recalibration might be set when worst-group
coverage drops more than 0.02--0.05 from nominal in production runs.
...
Since $\sigma_i$ is a deterministic function of the training data, the
normalized scores $s_i = |Y_i - \hat{f}(X_i)|/\sigma_i$ inherit
exchangeability over the calibration and test indices provided the underlying
examples $(X_i,Y_i)$ are exchangeable. Therefore the standard split-conformal
argument applies directly to the vector of normalized scores \citep{papadopoulos2002inductive,lei2018distribution,barber2023conformal}.

For the Mondrian variant we apply the same argument conditionally on each
type $t$: under Assumption (A3) exchangeability holds within type-$t$ groups,
so computing per-type quantiles yields the stated type-conditional (Mondrian)
guarantee.
\end{proof}
